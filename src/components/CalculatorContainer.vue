<template>
    ...
    <div class="grid grid-cols-5 gap-4 px-6 py-4">
      ...
      <button class="text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 shadow-lg shadow-red-500/50 dark:shadow-lg dark:shadow-red-800/80 font-medium rounded-lg text-sm py-2.5 text-center me-2 mb-2" @click="addSpace">Space</button>
    </div>

      <!-- Calculation History Menu -->
      <button @click="toggleHistory" class="text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-red-700 px-4 py-2 rounded-lg shadow-md transition duration-300 ease-in-out focus:outline-none"> {{ showHistory ? 'Hide History' : 'Show History' }} </button>

    <div v-if="showHistory" class="absolute left-1/2 transform -translate-x-1/2 mt-2 w-64 bg-red-600 text-white rounded-lg shadow-lg z-10">
      <h3 class="text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 shadow-lg shadow-red-500/50 dark:shadow-lg dark:shadow-red-800/80 px-4 py-3 rounded-t-lg font-semibold"> Calculation History </h3>
...
export default {
  ...
  data() {
    return {
          ...
          numeric: numeric,
          mode: 'Normal', 
          disableFunction: false ,// Initialize disableFunction to false
          history: [], // Initialize empty history array
          showHistory: false // Flag to toggle display of history dropdown
    };
  },
  methods: {
