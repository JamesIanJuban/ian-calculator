<template>
    ...
    <div class="container justify-center items-center p-8 relative bg-gradient-to-br from-gray-800 to-gray-900">
    <!-- Calculator display -->
    ...
    <div class="flex justify-end items-start mr-8 mt-4 mb-6">
     ...
     <!-- Calculation History Menu -->
     <div v-if="showHistory" class="absolute left-1/2 transform -translate-x-1/2 w-full h-40 overflow-y-auto bg-gradient-to-br from-gray-800 to-gray-900 text-white shadow-lg z-10 mt-6">
      <h3 class="text-white px-6 py-3 font-semibold bg-gradient-to-r from-red-400 via-red-500 to-red-600 transition-colors duration-300 hover:bg-red-700"> History </h3> 
      <ul class="divide-y divide-red-700"><li v-for="(calculation, index) in history.slice().reverse()" :key="index" class="px-4 py-3 transition duration-300 ease-in-out hover:bg-gradient-to-br hover:from-gray-900 hover:to-gray-800">{{ calculation }}</li></ul>
    </div>
    </div>
</template>
